...
void PCM1808_init(void)
{
    ...
    NVIC->IP[29] |= (1 << 11); // Preemptive Priority (2)! (max priority is 0)

//	NVIC->ISER[0] |= (1 << 29); // Enable Interrupt for I2S0

	I2S0->MDR |= (1 << 3) | (1 << 2); // Master Clock divide set as 6 (to create a period of 40 ns, 25 MHz, for the square wave, so device sample is 48 KHz -> 512fs)

//	I2S0->RCR2 |= (1 << 26) | (1 << 24); // Master Clock selected to generate Bit Clock, Bit Clock generated internally (master mode)
